...
): BlogFormType & { prefix: '/', miniature?: File } => {
  return {
    miniatureURL: myBlog?.miniature ?? '',
    miniature: undefined,
    title: myBlog?.title ?? '',
    prefix: '/',
    path: myBlog?.path != null ? extractWithoutPrefix(myBlog.path) : '',
...
export function MyBlogForm ({ handleCloseModal, handleNewGet, myBlog }: PropsType): JSX.Element {
  ...
  const {
    ...
    initialValues: initialValues(myBlog),
      ...
      try {
        ...
        if (myBlog == null) {
          ...
          await updateMyBlog(
            ...
            {
              title,
              content: (content !== myBlog.content ? content : undefined),
              path: (prefix.concat(path) !== myBlog.path ? prefix.concat(path) : undefined)
            },
            miniature
          )
        }
      ...
      <Grid container rowSpacing={{ xs: 1, sm: 2 }} columnSpacing={3}>
        <Grid xs={12} sm={12} display='flex' justifyContent='center'>
          <div {...getRootProps()}>
            ...
            >
              {values.miniatureURL === ''
                ? <PermMediaIcon sx={{ fontSize: '55px' }} />
                : null
              }
            </Avatar>
          </div>
