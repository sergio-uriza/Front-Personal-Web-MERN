import { CreateCourseBodyType, UpdateCourseBodyType } from '../types'
import { axiosConfig } from './config/axios.config'
import { SERVER_ROUTES } from './config/constants.config'
import { GetMultipleCourseType, MessageResponseType } from './types'

export const getMultipleCourse = async (limit = 10, page = 1): Promise<GetMultipleCourseType> => {
...
export const createCourse = async (accessToken: string | null, course: CreateCourseBodyType, miniature?: File): Promise<MessageResponseType> => {
...
export const updateCourse = async (accessToken: string | null, idCourse: string, course: UpdateCourseBodyType, miniature?: File): Promise<MessageResponseType> => {
  if (accessToken == null) throw new Error('Request Error, you do not have credentials')
...
export const deleteCourse = async (accessToken: string | null, idCourse: string): Promise<MessageResponseType> => {
  if (accessToken == null) throw new Error('Request Error, you do not have credentials')
